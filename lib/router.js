...
router.get('/logout',[checkToken],(req,res)=>{
        res.cookie("secureCookie", "expiredToken", {
            httpOnly: true,
            maxAge:0
          });
          res. header("Access-Control-Allow-Origin", "http://localhost:3000");

        res.status(200).send({token:"expiredToken"});
});
...
router.get('/getCharacter',[checkToken],(req,res)=>{
    CharacterService.getCharacterByUserId(req.userId).then((characterObj)=>{
        res.status(200).send({characterObj:characterObj});
    },(err)=>{
        res.status(400).send({message:err});
    });
});
router.post('/getWaifus',[checkToken],(req,res)=>{       //waifus of character
    CharacterService.getWaifus(req.body).then((waifus)=>{
        console.log("HALOOOOO",waifus)
...
router.post("/createWaifu",[checkToken],(req,res)=>{
    if(req.rightLevel>1){
        WaifuService.createWaifu(req.body.waifu).then(waifu=>{
            res.status(200).send({waifu:waifu});
        },(err)=>{
            console.log(err)
            res.status(400).send({message:err});
        });
